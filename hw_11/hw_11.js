// #### Task 1 💻
// У нас есть список постов на сервере.
// И наша задача отрисовать текст этих постов на странице.
// Но по каким то необъяснимым причинам, нам требуются посты номер 3, 7, 15, 23.
// Выглядит просто. Но есть нюанс 🐒
// Посты должны загружаться в определенном порядке.
// Сначала 15, потом 23, потом 7 и только потом 3.
// А если какой-то из постов не загрузиться, нам тогда необходимо  вывести в консоль ошибку.
// Наша задача должна иметь универсальное решение.
// В любой момент может потребоваться загрузить другие посты, или больше постов или меньше.
// ###### Реализуйте задачу двумя способами:
//   - Promise chaining
// - Async / await

//   - Promise chaining
function loadPost(postNumber) {
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (Math.random() > 0.1) { // успешная загрузка 90% времени
                resolve(`Post ${postNumber} content`)
            } else {
                reject(`Error loading post ${postNumber}`)
            }
        }, Math.random() * 1000) // случайная задержка до 1 секунды
    })
}

const postNumbers = [15, 23, 7, 3]
loadPost(postNumbers[0])
    .then(result => {
        console.log(result)
        return loadPost(postNumbers[1])
    })
    .then(result => {
        console.log(result)
        return loadPost(postNumbers[2])
    })
    .then(result => {
        console.log(result)
        return loadPost(postNumbers[3])
    })
    .catch(error => {
        console.error(error)
    })

// - Async / await

async function loadPosts() {
    for (const postNumber of postNumbers) {
        try {
            const result = await loadPost(postNumber)
            console.log(result)
        } catch (error) {
            console.error(error)
        }
    }
}

loadPosts()

// #### Task 2 💻
// + Создайте ветку на github `name-fetch-training`
// + Далее создайте папку `fetch-training` и добавьте файлы `index.html` `index.js`
// + Реализуйте приложение:

// + Перейдите по ссылке `https://jsonplaceholder.typicode.com/`
// + Во вкладке `Resources` лежит ключ `/todos` - Это именно те данные которые нам нужны
// + Далее напишите две функции `getTodos` & `printTodos`
// + Функция `getTodos` делает запрос по указанному адресу и забирает данные.
// + Функция `printTodos` создает список массив объектов, объект записывает
// id и title объекта с делом
// + Вывести полученный результат в консоль