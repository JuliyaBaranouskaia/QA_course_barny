...
console.log(getSum1(num))

//Task 2 💻
//Напишите функцию которая в качестве аргумента принимает в себя сумму
// кредита который хочет получить клиент и верните результат переплаты по кредиту:
//+ процентная ставка в год — 17%,
//+ количество лет — 5.
//Мы пишем функцию для хорошего банка, поэтому сумма кредита не увеличивается.

function creditCalculation(sum) {
    const procentYear = 0.17
    const years = 5
    let totalPayment = sum * (1 + procentYear * years)
    return totalPayment - sum
}

console.log(`Переплата по кредиту составит: ${creditCalculation(100)}`)
...
let originalString = 'тестовая строка для проверки функции'
let trimmedString = trimString(originalString, 9, 16)
...
function getSumNumbers(numberTest) {
    let sum = 0
    for (let numberExc of numberTest.toString()) {
        sum += Number(numberExc) //  цифру обратно в число и добавляю к сумме
    }
    return sum
}

let year = 2021
console.log(getSumNumbers(year))
...
function fooboo(booleanType, foo, boo) {
    if (booleanType) {
        foo() // foo, если condition true
    } else {
        boo() // boo, если condition false
    }
}
...
const factorialCalculation = (function() {
    return function(num) {
        if (num < 0) {
        ...
        let resultOfCalculation = 1
        for (let i = 2; i <= num; i++) {
            resultOfCalculation *= i
        }
